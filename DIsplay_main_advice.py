...
from compare_audio import compare_audio,rhythm_compare,segmented_pitch_and_intonation
...
def post_recording():
    ...
    scores, plot_labels, plot_range,sum_score,spi_ave,si_ave = compare(target_files, Weights)
    update_status("採点終了！")
    plot_radar_chart(scores, plot_labels,plot_range, save_file)
    root.after(500, result(scores,sum_score,spi_ave,si_ave))
...
def compare(target_files, Weights):
    ...
    spi_ave = np.mean(segmented_pitch,axis = 0)
    si_ave = np.mean(segmented_intonation,axis = 0)
    ...
    return scores, plot_labels,plot_range,Score,spi_ave,si_ave
...
def result(scores,sum_score,spi_ave,si_ave):
    clear_screen()
...
# 左側のフレーム (点数を表示)
    ...
    segments = ["序盤", "中盤", "終盤"]
    # --- Generate feedback ---
    min_pitch_index = np.argmin(spi_ave)
    min_intonation_index = np.argmin(si_ave)

    min_pitch_similarity = min(spi_ave)
    min_intonation_similarity = min(si_ave)
    ...
    if min_pitch_similarity < min_intonation_similarity:
        advice_2 = f"特に {segments[min_pitch_index]} の音高に修正が必要です。"
    else:
        advice_2 = f"特に {segments[min_intonation_index]} の音量に修正が必要です。"
    ...
    advice_label.pack(pady=20, anchor="w")
